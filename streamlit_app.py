...
# URL už nie je v kóde, ale v tajnom nastavení Streamlitu
# Ak vyvíjaš lokálne, vytvor si súbor .streamlit/secrets.toml
try:
    SCRIPT_URL = st.secrets["SCRIPT_URL"]
except Exception:
    st.error("Chýba konfigurácia SCRIPT_URL v Secrets!")
    st.stop()
            ...
            if nac.empty:
                ...
                for i, row in nac.iterrows():
                    ...
                    with st.expander(f"Zákazník: {row.get('poznamka')} | Partner: {m} {p}"):
                        ...
                        st.write(f"Telefón: {row.get('mobil_partnera', '---')}")
